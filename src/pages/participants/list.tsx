  ...
  const renderContent = () => (
    <Box sx={{ 
      ...
      <Grid container spacing={isMobile ? 2 : 3} sx={{ mb: isMobile ? 2 : 3 }}>
        ...
        <Grid item xs={6} sm={6} md={3}>
          <StatCard title={t('common.active')} value={allParticipants.filter(p => p.is_active).length.toString()} icon={<VerifiedUserIcon sx={{ fontSize: isMobile ? MOBILE_ICON_SIZE : DESKTOP_ICON_SIZE }} />} color="success" />
        </Grid>
        <Grid item xs={6} sm={6} md={3}>
          <StatCard title={t('status_options.inactive')} value={allParticipants.filter(p => !p.is_active).length.toString()} icon={<EmailIcon sx={{ fontSize: isMobile ? MOBILE_ICON_SIZE : DESKTOP_ICON_SIZE }} />} color="error" />
        </Grid>
        <Grid item xs={6} sm={6} md={3}>
